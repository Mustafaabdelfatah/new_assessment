...
@if ($theme === 'tailwind')
    <th scope="col"
        {{ $attributes->merge($customAttributes)->class([
            ...
            ])->class(['hidden sm:table-cell' => $column->shouldCollapseOnMobile()])->class(['hidden md:table-cell' => $column->shouldCollapseOnTablet()])->except('default') }}>
        @unless($component->sortingIsEnabled() && $column->isSortable())
            {{ $column->getTitle() }}
        @else
            <button wire:click="sortBy('{{ $column->getColumnSelectName() }}')"
                {{ $attributes->merge($customSortButtonAttributes)->class([
                        'flex items-center space-x-1 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider group focus:outline-none dark:text-gray-400' =>
...
@elseif ($theme === 'bootstrap-4' || $theme === 'bootstrap-5')
    <th scope="col"
        ...
        @unless($component->sortingIsEnabled() && $column->isSortable())
            {{ $column->getTitle() }}
        @else
            <div class="min-w-125px" wire:click="sortBy('{{ $column->getColumnSelectName() }}')" style="cursor:pointer;">
                <span>{{ $column->getTitle() }}
                    <span class="ms-2">
                        @if ($direction === 'asc')
                            <svg xmlns="http://www.w3.org/2000/svg" class="ml-1" style="width:1em;height:1em;"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
